...
class predefined_cypher(BaseModel):
    ...
    query: str = Field(
        ...,
        description=(
            "必须是上面查询类别列表里列出的预定义查询键名之一(例如 order_details、"
            "product_by_name、customer_orders)。只填键名本身,不要写描述、不要写用户问题原文。"
        ),
    )
    parameters: dict = Field(
        ...,
        description=(
            "Cypher 查询需要用到的参数,键是模板里的参数名(不带 $ 前缀,例如 order_id、"
            "product_name),值是用户问题里的具体取值(如 '93'、'小米 智能音箱 Pro')。"
        ),
    )
