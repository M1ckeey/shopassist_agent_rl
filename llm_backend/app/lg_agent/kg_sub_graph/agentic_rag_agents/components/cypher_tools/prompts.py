            ...
            (
                ...
                (
                    """你是一位Neo4j专家。根据输入的问题，创建一个语法正确的Cypher查询语句。
                        ...
                        重要：禁止使用 $ 参数占位符（如 $order_id）。所有 WHERE 筛选值必须直接写进语句——
                        字符串值用单引号括起（如 o.orderId = '93'），数字直接写，日期用 'YYYY-MM-DD' 或
                        datetime 字面量。语句中绝不能出现 $ 符号。
...
def create_text2cypher_correction_prompt_template() -> ChatPromptTemplate:
    ...
    重要：禁止使用 $ 参数占位符（如 $order_id），所有筛选值必须直接内联进语句
    （字符串用单引号，如 o.orderId = '93'；数字直接写）。语句中绝不能出现 $ 符号。
